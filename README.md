...
- **运行时导入账号** — `POST /admin/accounts/ingest`（或同路径 WebSocket）上传 JSON / NDJSON，直接填充号池，无需重启
...
| [docs/API-ADMIN.md](docs/API-ADMIN.md) | 管理类接口：`/stats`、`/refresh`、**账号导入**、401 恢复等 |
...
除将 JSON 放入 `auth-dir` 外，也可在服务运行后通过管理接口导入，见 [docs/API-ADMIN.md](docs/API-ADMIN.md) 中的 **`/admin/accounts/ingest`**。
...
curl http://localhost:8080/v1/images/generations \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer sk-your-custom-key" \
  -d '{"model": "gpt-5.5-image", "prompt": "A cute robot painting a sunset", "response_format": "b64_json"}'
```
...
- **`-image`**：注入上游 `image_generation` 工具；`/v1/images/generations` 会默认使用 `gpt-5.5-image` 并内部走该模式。
...
大上下文示例：`gpt-5.5-1m`、`gpt-5.5-high-1m-fast`、`gpt-5.5-xhigh-1m-fast`。
